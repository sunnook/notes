...
【接收/组帧】RtpVideoStreamReceiver2（实现 RtpPacketSinkInterface）// video/rtp_video_stream_receiver2.h
  │  OnRtpPacket(:759) → ReceivePacket(:765) → (:1230)OnReceivedPayloadData(:546)
  │  → packet_buffer_.InsertPacket(:742)      // PacketBuffer 拼包（把 RTP 包组完整帧）
  │    → OnInsertedPacket(:812) → OnAssembledFrame(:916)  // 拼成 RtpFrameObject(EncodedFrame)
  │      → reference_finder_->ManageFrame(:979)  // RtpFrameReferenceFinder 帧间引用重排序
...
```
【采集】麦克风 → AudioDeviceModule(ADM) 采集 PCM
  │  ADM 通过 AudioTransport 回调把采集数据交给 AudioState
  ▼
...
【call 层】AudioSendStream 构造函数                         // audio/audio_send_stream.cc:121
  ...
  ▼
【编码】ChannelSend::ProcessAndEncodeAudio                  // audio/channel_send.cc:201
  │  → AudioCodingModule 编码（audio/channel_send.cc:280）
  │    → AudioEncoder（Opus），含 AudioProcessing(AEC/NS/AGC) 前处理
  │  → SendData → SendRtpAudio                             // :233, :408
  ▼
...
【解码】NetEqImpl::GetAudio / GetAudioInternal               // neteq_impl.cc:432, :663
  ...
  ▼
【播放】ChannelReceive::GetAudioFrameWithInfo                // audio/channel_receive.cc:195
...
【worker 线程启动即循环】Thread::Run()(:734) → ProcessMessages(kForever)(:735)
  ...
  → Dispatch(std::move(task))(:876)                  // 执行任务
...
| 拥塞控制 | `ProcessThread` 周期 `OnProcessInterval`（默认 25ms） | modules/congestion_controller/goog_cc/goog_cc_network_control.cc:191 |
