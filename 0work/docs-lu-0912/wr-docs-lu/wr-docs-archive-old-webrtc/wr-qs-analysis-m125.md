> ⚠️ **【已归档·老版本】本文档基于老版 WebRTC（推测约 M105–M125）编写，非当前 M144（`wr-m144_release`）。**
> **判定证据（与 M144 源码树对拍，2026-09-10）**：
> 1. `NackModule` 出现 23 次、`NackRequester` 0 次 —— M144 里 NACK 模块叫 `NackRequester`（`modules/video_coding/nack_requester.{cc,h}`），不存在 `NackModule`。
> 2. 引用 `modules/pacing/paced_sender.h`（`PacedSender`）—— M144 无此文件，实际为 `TaskQueuePacedSender`（`modules/pacing/task_queue_paced_sender.cc`）+ 核心算法 `PacingController`（`pacing_controller.cc`）。
> 3. 引用 `modules/congestion_controller/rtp/include/receive_side_cc.h` —— M144 无此文件，实际为 `modules/congestion_controller/include/receive_side_congestion_controller.h`。
> 4. 引用 `video/rtp_video_stream_receiver.cc` —— M144 无此文件，实际为 `video/rtp_video_stream_receiver2.cc`。
> 5. 引用 `modules/include/module.h` —— M144 树中不存在该路径。
>
> **版本推测**：类名/文件结构对应 WebRTC 约 M105–M125 时代（`NackModule`、独立 `paced_sender.h`、`VCMJitterEstimator` 等命名特征），且文中混杂部分较新概念（TWCC、`PacingController`、`RobustThroughputEstimator`），判断为基于约 M125 前后的老树编写、后经局部补注。
>
> **M144 修订版**：见 `../wr-qs-analysis-m144.md`（逐章节对照 `wr-m144_release` 核对修正）。
