...
- `decreased = estimated_throughput * beta_(0.85)` —— **乘性减少，因子 0.85**；有链路容量估计时用 `beta_ * link_capacity_.estimate()`（`:300`）。
...
RTP 包 (abs-send-time 扩展)
  ...
  │     T = min(num_deltas, 60)·offset, 阈值自适应(c), 夹在 [6,600]
  │     if T > threshold 且持续>10ms 且 overuse_counter>1: Overusing
  │
...
状态转移 F = I (随机游走), 过程噪声 Q 加到 P 对角线
观测 H = [1, 1]→实际 h = [fs_delta, 1]   // 首分量是"帧大小增量", 非时间差
预测: x̂ = F·x̂, P = F·P·Fᵀ + Q
残差: y = t_ts_delta - slope·h[0] - offset
      其中 t_ts_delta = t_delta - ts_delta  // 到达间隔减发送间隔(补偿)
新息: K = P·Hᵀ / (H·P·Hᵀ + R),  R = 测量噪声 var_noise_
更新: x̂ += K·y, P = (I - K·H)·P
```

**M144 实测（`overuse_estimator.cc:30-103`）**：观测矩阵首分量是**帧大小增量 `fs_delta`** 而非 Δts；残差基于**发送补偿后的到达间隔 `t_ts_delta = t_delta - ts_delta`**；三处的 `R` 就是测量噪声 `var_noise_`（**无 ×25 因子**，`cc:74` `denom = var_noise_ + h[0]·Eh[0] + h[1]·Eh[1]`）；噪声更新用 3σ 门限滤掉非常迟到的帧（`max_residual = 3.0·sqrt(var_noise_)`，`cc:65-71`）；`num_of_deltas_` 仅用于封顶 `kDeltaCounterMax=1000`，overuse/underuse 转换时 `E[1][1] += 10·process_noise_[1]` 陡增第二条对角线过程噪声（`cc:47-51`），并非持续衰减 Q。

#### 3.3.2 OveruseDetector 自适应阈值

`overuse_detector.cc` 的阈值随时间自适应（`UpdateThreshold`，`cc:79-96`）：

```
k = |offset| < threshold ? kDown : kUp       // 视当前阈值内侧/外侧选择系数
threshold += k · (|offset| − threshold) · time_delta_ms
  time_delta_ms = min(now − last_update, 100ms);   // kMaxTimeDeltaMs=100
threshold = clamp(threshold, 6, 600)               // SafeClamp[:6,600]
/* 若 |offset| > threshold + kMaxAdaptOffsetMs(15): 视为大延迟尖峰, 跳过本次更新 */
```

**M144 实测（`overuse_detector.cc:23-27`）**：`kUp = 0.0087`、`kDown = 0.039`。因为 kUp(0.0087) < kDown(0.039)：**阈值慢升(kUp)快降(kDown)**——长期稳定时缓慢放宽到稳态阈值，出现过用时快速收紧。检测用 `T = min(num_deltas, kMaxNumDeltas=60)·offset`（`cc:45`），过用需同时满足 `time_over_using_ > kOverUsingTimeThreshold(10)`、`overuse_counter_ > 1`、`offset >= prev_offset_`（`cc:47-66`）。
...
`Push`（`h:50`）按优先级入队，高优先级先发。队列还支持**按优先级 TTL**（`PacketQueueTTL`：audio_retransmission/video_retransmission 等，`cc:67-77`），超时丢包防过期数据占用。

#### 4.4.2 StreamQueue 流间公平

同一优先级的多个流以 `StreamQueue` 组织（`h:184`——每优先级一组有包的 StreamQueue），`UpdateAverageQueueTime` 轮转调度：同优先级内各流近似轮流发包，保证流间近似按比例共享带宽。每个 `QueuedPacket` 记录 `enqueue_time`（`h:113`）用于队列时长统计。
...
ProbeController 调度 → Pacer::CreateProbeCluster(config)
  │ config = {target_rate(send_bitrate), min_probes, min_bytes, min_probe_delta}
  ▼
...
// 关键帧加 firstPartitionProt(~20%) boost (cc:333-334)
...
⚠️ **版本差异**：老版 `FrameBuffer`（`modules/video_coding/frame_buffer2.{h,cc}`，带 `NextFrame` 阻塞等待 + `crit_`/`frame_event_`）在 M144 已被**重写为时间单元（temporal unit）模型**：`api/video/frame_buffer.{h,cc}`（2021 年重写），无锁（thread-unsafe，外部串行化）、按 frame ID 排序、按时间戳分组；等待/调度逻辑外移到 `video/video_stream_buffer_controller.{h,cc}` + `video/frame_decode_scheduler.{h,cc}` + `video/frame_decode_timing.{h,cc}`。旧 `VCMJitterBuffer`/老 `FrameBuffer` 移到 `modules/video_coding/deprecated/`。
...
依赖永远不到（丢包未恢复）时，由上层 `VideoStreamBufferController`（`video/video_stream_buffer_controller.{h,cc}`）负责：等待超时后 `DropNextDecodableTemporalUnit` 跳帧，或在关键帧请求后清空重来。帧缓冲本身不阻塞——调度/超时/等待在 `video/frame_decode_scheduler.{h,cc}` + `video/frame_decode_timing.{h,cc}`。
...
| `FrameDecodeScheduler` | decode 队列 | `video/frame_decode_scheduler.{h,cc}`，负责等待/超时唤醒（替代老版 `frame_event_` ConditionVar） |
...
CheckQpTask (cc:80, 周期检查, 同一时刻只跑一个):
  ...
  if avg_qp_high > thresholds_.high:   → CheckQpResult::kDecrease (cc:302)
  elif avg_qp_low < thresholds_.low    → kIncrease
...
- **驱动**：`FrameDecodeScheduler`（`video/task_queue_frame_decode_scheduler.{h,cc}`）按解码时间 `PostDelayedTask` 精确唤醒，不轮询
...
⚠️ **音频开销扣除**：老 trial `WebRTC-SendSideBwe-WithOverhead` 已删除——M144 的 `BitrateController` **默认**就做开销扣除（`bitrate_controller.cc:46-62`），不再由开关控制。
