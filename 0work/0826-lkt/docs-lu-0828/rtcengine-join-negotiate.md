...
```text
参与者：
  App       Engine      Signal      PCT(pub)      Obs PC(lib)      L5 Observer      Server

阶段一：加入房间

App -> Engine: join()
Engine -> Signal: join()
Signal -> Server: JoinReq
Server --> Signal: JoinResp
Signal --> Engine: joinResp
Engine --> App: joined

阶段二：配置发布端 PeerConnection

App -> Engine: configure()
Engine -> PCT(pub): create()
PCT(pub) -> Obs PC(lib): createPeerConnection()
PCT(pub) -> L5 Observer: register(pcObserver)
PCT(pub) -> Obs PC(lib): createDataChannel()
Obs PC(lib) --> PCT(pub): DataChannel

阶段三：协商 Publisher

App -> Engine: negotiatePublisher()
Engine -> PCT(pub): negotiate(constraints)
PCT(pub) -> PCT(pub): async delay / task scheduling
PCT(pub) -> Obs PC(lib): createOffer()
Obs PC(lib) --> PCT(pub): offer(SDP)
PCT(pub) -> Obs PC(lib): setLocalDescription(offer)
PCT(pub) -> L5 Observer: onOffer(localDescription)
L5 Observer -> Signal: sendOffer(offer)
Signal -> Server: Offer

阶段四：接收 Answer 并设置远端描述

Server --> Signal: Answer
Signal --> L5 Observer: onServerAnswer(answer)
L5 Observer -> PCT(pub): setRemoteDescription(answer)
PCT(pub) -> Obs PC(lib): setRemoteDescription(answer)

阶段五：ICE Trickle

Obs PC(lib) -> PCT(pub): onIceCandidate(candidate)
PCT(pub) -> L5 Observer: onIceCandidate(candidate)
L5 Observer -> Signal: sendCandidate(candidate)
Signal -> Server: TrickleCandidate
Server --> Signal: onTrickle(candidate)
Signal --> L5 Observer: onTrickle(candidate)
L5 Observer -> PCT(pub): addIceCandidate(candidate)
PCT(pub) -> Obs PC(lib): addIceCandidate(candidate)

阶段六：连接建立

Obs PC(lib) -> PCT(pub): onConnectionChange(connected)
PCT(pub) -> Engine: connectionChangeListener(connected)
Engine -> App: engineConnected
App -> App: connectionState = CONNECTED
```

建议同时做以下调整：

1. 将 `Engine` 与 `PCT(pub)` 的职责分开：`Engine` 负责业务状态和流程编排，`PCT(pub)` 负责 WebRTC PeerConnection 生命周期。
2. 将 `L5 Observer` 命名为 `PC Observer / L5 Adapter`，避免看不出它是回调适配层。
3. `createOffer()`、`setLocalDescription()`、`setRemoteDescription()`、`addIceCandidate()` 都建议明确标注为异步操作，并在实现中等待完成后再进入下一步。
4. `onIceCandidate` 与 Answer 返回是并发事件，不能画成严格的线性依赖。ICE candidate 可能在设置远端描述之前产生，需要缓存，待 `setRemoteDescription()` 完成后再添加。
5. `connected` 不建议直接等同于业务层最终成功。可以按以下状态更新：
   `NEW -> JOINED -> CONFIGURED -> NEGOTIATING -> CONNECTING -> CONNECTED`。
6. 连接失败、断开和重连路径应补充：
   `failed`、`disconnected`、`closed`，否则图中只有成功路径，无法覆盖实际运行时行为。

如果需要用 Mermaid 表达，可以写成：
...
sequenceDiagram
    ...
    participant Engine
    participant Signal
    participant PCT as PCT(pub)
    participant PC as Obs PC(lib)
    participant Adapter as L5 PC Adapter
    participant Server

    rect rgb(235, 245, 255)
        Note over App,Server: Join
        App->>Engine: join()
        Engine->>Signal: join()
        Signal->>Server: JoinReq
        Server-->>Signal: JoinResp
    ...
    rect rgb(238, 250, 240)
        Note over App,PC: Configure Publisher PC
        App->>Engine: configure()
        Engine->>PCT: create()
        PCT->>PC: createPeerConnection()
        PCT->>Adapter: register(pcObserver)
        PCT->>PC: createDataChannel()
        PC-->>PCT: dataChannel
    end

    rect rgb(255, 247, 230)
        Note over App,Server: Publisher Negotiation
        App->>Engine: negotiatePublisher()
        Engine->>PCT: negotiate(constraints)
        PCT->>PC: createOffer()
        PC-->>PCT: offer(SDP)
        PCT->>PC: setLocalDescription(offer)
        PCT->>Adapter: onOffer(offer)
        Adapter->>Signal: sendOffer(offer)
        Signal->>Server: Offer
        Server-->>Signal: Answer
        Signal-->>Adapter: onServerAnswer(answer)
        Adapter->>PCT: setRemoteDescription(answer)
        PCT->>PC: setRemoteDescription(answer)
    end

    rect rgb(250, 240, 250)
        Note over PC,Server: Trickle ICE, concurrent with Answer
        par Local candidate
            PC-->>PCT: onIceCandidate(candidate)
            PCT-->>Adapter: onIceCandidate(candidate)
            Adapter->>Signal: sendCandidate(candidate)
            Signal->>Server: TrickleCandidate
        and Remote candidate
            Server-->>Signal: onTrickle(candidate)
            Signal-->>Adapter: onTrickle(candidate)
            Adapter->>PCT: addIceCandidate(candidate)
            PCT->>PC: addIceCandidate(candidate)
        end
    end

    rect rgb(245, 245, 245)
        Note over PC,App: Connected
        PC-->>PCT: onConnectionChange(connected)
        PCT-->>Engine: connectionChangeListener(connected)
        Engine-->>App: engineConnected
        App->>App: state = CONNECTED
    end
```

其中最重要的结构性修正是：把 ICE Trickle 画成与 Answer 并发的事件流，并在实现层处理 candidate 缓存和时序问题。
